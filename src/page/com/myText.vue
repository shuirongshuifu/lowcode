<template>
    <div class="targetDom" :style="styleComputed" ref="targetDom" @click="chooseFn" @input="inputFn"
...
const styleComputed = computed(() => {
    ...
    props.style.letterSpacing ? props.style.letterSpacing = props.style.letterSpacing + 'px' : null
    props.style.wordSpacing ? props.style.wordSpacing = props.style.wordSpacing + 'px' : null
    return props.style
})
...
const setDomCanDrag = (dom, info) => {
    ...
    props.container.addEventListener("mousemove", (e) => {
        ...
        // 根据鼠标移动的偏移量来更新当前位置
        info.currentX =
            info.lastDragEndX + (e.clientX - info.startX);
        info.currentY =
            info.lastDragEndY + (e.clientY - info.startY);
        // 使用transform来更新位置和缩放
...
.targetDom:focus-visible {
    outline: 1.5px dashed #007bff;
    outline-offset: 5px;
}
</style>